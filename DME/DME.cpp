...
Point<DBU> DME::computeMergePoint(const MergeSegment &ms1, const MergeSegment &ms2, const Point<DBU> *cluster_center,
     ...
     */
    DBU dx =
        (cluster_center->get_x() - left_merge_point->get_x()) * (cluster_center->get_x() - right_merge_point->get_x());
    DBU dy =
        (cluster_center->get_y() - left_merge_point->get_y()) * (cluster_center->get_y() - right_merge_point->get_y());
...
void DME::computeTpClusterCenterLocation(ClusterTopo &binary_tree, const string &lcb_name) {
    /**
    ...
    if (sink_num == 0) {
        cout << "[CTS-flow] Error: The Cluster " << lcb_name << "has not sink." << endl;
        return;
    }
    ...
    while (index >= 0) {
        ...
        if (!left_vertex->isUnLegal() && !right_vertex->isUnLegal()) {
            ...
            if (manhattan_dist >= skew_dist) {
                ...
                MergeSegment left_ms = make_pair(new Point<DBU>(left_x + direction_x * radius_left, left_y),
                                                 new Point<DBU>(left_x, left_y + direction_y * radius_left));
                MergeSegment right_ms = make_pair(new Point<DBU>(right_x - direction_x * radius_right, right_y),
                                                  new Point<DBU>(right_x, right_y - direction_y * radius_right));

                Point<DBU> *merge_point = computeMergePoint(left_ms, right_ms, center, lcb_name);

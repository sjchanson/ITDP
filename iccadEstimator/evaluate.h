...
struct layer {
    ...
    layer() : name(""), type(""), direction(""), xPitch(0.0), yPitch(0.0), xOffset(0.0), yOffset(0.0), width(0.0) {}
    void print();
};
...
struct pin {
    ...
    pin()
        ...
        , rTran(0.0)
        , fTran(0.0)
        , driverType(numeric_limits<unsigned>::max())
        , x_coord(0.0)
        , y_coord(0.0)
        , x_offset(0.0)
        , y_offset(0.0)
...
struct row {
    ...
    row()
        : name("")
        , site(numeric_limits<unsigned>::max())
        , origX(0)
        , origY(0)
        , stepX(0)
        , stepY(0)
...
private:
    ...
    /* add by shijianchen, plan to get flipflop connection information */
    uit::Timer timer;

public:
    circuit()
        : num_fixed_nodes(0)
        , LOCAL_WIRE_CAP_PER_MICRON(0.20e-15)
        , LOCAL_WIRE_RES_PER_MICRON(0.60)

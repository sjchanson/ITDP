    ...
    do {
        ...
        if (valid) {
            ...
            for (unsigned i = 0; i < pinNetPairs.size(); ++i) {
                ...
                if (cellType == "INV_Z80" && myNet->name == "iccad_clk") myCell->isLCB = true;

                assert(macro2id.find(cellType) != macro2id.end());
                assert(macros[macro2id[cellType]].pins.find(pinNetPairs[i].first) !=
                       macros[macro2id[cellType]].pins.end());
...
void circuit::read_def(const string &input, bool mode) {
    ...
    while (!dot_def.eof()) {
        ...
        if (tokens[0] == "VERSION") {
            ...
            get_next_token(dot_def, tokens[0], DEFCommentChar);
            while (tokens[0] != "PROPERTYDEFINITIONS") get_next_token(dot_def, tokens[0], DEFCommentChar);
        } else if (tokens[0] == "SPECIALNETS") {
            get_next_token(dot_def, tokens[0], DEFCommentChar);
            while (tokens[0] != "SPECIALNETS") get_next_token(dot_def, tokens[0], DEFCommentChar);
...
void circuit::read_lef_macro(ifstream &is) {
    ...
    while (tokens[0] != "END") {
        if (tokens[0] == "CLASS") {
            ...
            get_next_token(is, tokens[0], LEFCommentChar);
            while (tokens[0] != LEFLineEndingChar) get_next_token(is, tokens[0], LEFCommentChar);
            assert(tokens[0] == LEFLineEndingChar);
...
void net::print() {
    ...
    for (unsigned i = 0; i < sinks.size(); ++i) cout << "sink pin [" << i << "]:      " << sinks[i] << endl;
    cout << "|===  END  NET ===|" << endl;
}
...
void circuit::write_bookshelf() {
    ...
    if (!dot_aux.good()) return;
    dot_aux << "RowBasedPlacement : " << benchmark << ".nodes " << benchmark << ".nets " << benchmark << ".wts "
            << benchmark << ".pl " << benchmark << ".scl " << benchmark << ".shapes";
    dot_aux.close();

    // write .nodes (including PI & PO pins as instances)
    ofstream dot_nodes((bookshelf_dir + benchmark + ".nodes").c_str());
    if (!dot_nodes.good()) return;
    ...
    for (vector<unsigned>::iterator PI = PIs.begin(); PI != PIs.end(); ++PI) {
        if (pins[*PI].name == clock_port) continue;
    ...
    for (vector<cell>::iterator theCell = cells.begin(); theCell != cells.end(); ++theCell) {
        if (theCell->isFixed)
            dot_nodes << "\t\t" << theCell->name << "\t\t" << theCell->width / BOOKSHELF_X_SCALE << "\t\t"
                      << theCell->height / BOOKSHELF_Y_SCALE << "\t terminal" << endl;
        else
            dot_nodes << "\t\t" << theCell->name << "\t\t" << theCell->width / BOOKSHELF_X_SCALE << "\t\t"
                      << theCell->height / BOOKSHELF_Y_SCALE << endl;
    }
    dot_nodes.close();

    // write .nets
    ofstream dot_nets((bookshelf_dir + benchmark + ".nets").c_str());
    if (!dot_nets.good()) return;
    ...
    for (vector<pin>::iterator thePin = pins.begin(); thePin != pins.end(); ++thePin) {
        if (nets[thePin->net].name == clock_port) continue;
        pin_cnt++;
    }
    ...
    for (vector<net>::iterator theNet = nets.begin(); theNet != nets.end(); ++theNet) {
        ...
        if (pins[theNet->source].type == PI_PIN)
            ...
            dot_nets << pins[theNet->source].x_offset / BOOKSHELF_X_SCALE - 0.5 * pinOwner->width / BOOKSHELF_X_SCALE
                     << "\t"
                     << pins[theNet->source].y_offset / BOOKSHELF_Y_SCALE - 0.5 * pinOwner->height / BOOKSHELF_Y_SCALE
                     << endl;
        }
        for (vector<unsigned>::iterator theSink = theNet->sinks.begin(); theSink != theNet->sinks.end(); ++theSink) {
            if (pins[*theSink].type == PO_PIN)
                ...
                dot_nets << pins[*theSink].x_offset / BOOKSHELF_X_SCALE - 0.5 * pinOwner->width / BOOKSHELF_X_SCALE
                         << "\t"
                         << pins[*theSink].y_offset / BOOKSHELF_Y_SCALE - 0.5 * pinOwner->height / BOOKSHELF_Y_SCALE
                         << endl;
            }
        }
    }
    ...
    for (unsigned i = 0; i < rows.size(); i++) {
        ...
        dot_scl << "   SubrowOrigin   : " << theRow->origX / BOOKSHELF_X_SCALE
                << "\t\t\tNumSites : " << theRow->numSites << endl;  //* theRow->stepX << endl;
        while (i + 1 <= rows.size() - 1) {
            if (rows[i + 1].origY == theRow->origY) {
                dot_scl << "   SubrowOrigin   : " << rows[i + 1].origX / BOOKSHELF_X_SCALE
                        << "\t\t\tNumSites : " << rows[i + 1].numSites * rows[i + 1].stepX << endl;
                i++;
            } else
                break;
        }
    ...
    if (!dot_pl.good()) return;
    dot_pl << "UCLA pl 1.0" << endl << endl;
    for (vector<unsigned>::iterator PI = PIs.begin(); PI != PIs.end(); ++PI) {
        if (pins[*PI].name == clock_port) continue;
        dot_pl << "\t\t" << pins[*PI].name << "\t\t" << pins[*PI].x_coord / BOOKSHELF_X_SCALE << "\t\t"
               << pins[*PI].y_coord / BOOKSHELF_Y_SCALE << "\t : N" << endl;
    }
    for (vector<unsigned>::iterator PO = POs.begin(); PO != POs.end(); ++PO)
        dot_pl << "\t\t" << pins[*PO].name << "\t\t" << pins[*PO].x_coord / BOOKSHELF_X_SCALE << "\t\t"
               << pins[*PO].y_coord / BOOKSHELF_Y_SCALE << "\t : N" << endl;
    for (vector<cell>::iterator theCell = cells.begin(); theCell != cells.end(); ++theCell) {
        if (theCell->isFixed)
            dot_pl << "\t\t" << theCell->name << "\t\t" << theCell->init_x_coord / BOOKSHELF_X_SCALE << "\t\t"
                   << theCell->init_y_coord / BOOKSHELF_Y_SCALE << "\t : N" << endl;
        else
            dot_pl << "\t\t" << theCell->name << "\t\t" << theCell->init_x_coord / BOOKSHELF_X_SCALE << "\t\t"
                   << theCell->init_y_coord / BOOKSHELF_Y_SCALE << "\t : N" << endl;
    }
    ...
    if (!dot_shapes.good()) return;

    unsigned num_nonrect_nodes = 0;
    for (vector<cell>::iterator theCell = cells.begin(); theCell != cells.end(); ++theCell)
        if (macros[theCell->type].obses.size() > 1) num_nonrect_nodes++;

    dot_shapes << "shapes 1.0" << endl << endl;
    dot_shapes << "NumNonRectangularNodes : " << num_nonrect_nodes << endl << endl;
    for (vector<cell>::iterator theCell = cells.begin(); theCell != cells.end(); ++theCell) {
        if (!theCell->isFixed || macros[theCell->type].obses.size() <= 1) continue;
        int cnt = 0;
        ...
        for (vector<rect>::iterator theRect = macros[theCell->type].obses.begin();
             theRect != macros[theCell->type].obses.end(); ++theRect, ++cnt)
            dot_shapes << "		Shape_" << cnt << " "
                       << ((unsigned)(theCell->init_x_coord +
                                      (unsigned)(theRect->xLL * static_cast<double>(LEFdist2Microns)))) /
                              BOOKSHELF_X_SCALE
                       << " "
                       << ((unsigned)(theCell->init_y_coord +
                                      (unsigned)(theRect->yLL * static_cast<double>(LEFdist2Microns)))) /
                              BOOKSHELF_Y_SCALE
                       << " "
                       << ((unsigned)((theRect->xUR - theRect->xLL) * static_cast<double>(LEFdist2Microns))) /
                              BOOKSHELF_X_SCALE
                       << " " << rowHeight / BOOKSHELF_Y_SCALE << endl;
    }
    dot_shapes.close();
    ...
    while (!dot_pl.eof())  // for(unsigned i=0;i<cells.size()+PIs.size()+POs.size()-1;i++)
    {
        ...
        if (pin2id.find(tmpStr) != pin2id.end()) {
            ...
            if (cells[it->second].isFixed)
                dot_def << "      + FIXED ( " << cells[it->second].init_x_coord << " " << cells[it->second].init_y_coord
                        << " ) N ;" << endl;
            else
                dot_def << "      + PLACED ( " << x_coord * BOOKSHELF_X_SCALE << " " << y_coord * BOOKSHELF_Y_SCALE
                        << " ) N ;" << endl;
        }
        dot_pl >> tmpStr;
        if (tmpStr == "/FIXED")
...
void circuit::read_ops(const string &input) {
    ...
    bool passed = true;
    for (vector<pin>::iterator thePin = pins.begin(); thePin != pins.end(); ++thePin) {
        if (!thePin->isFlopCkPort) continue;
    ...
    for (vector<cell>::iterator theCell = cells.begin(); theCell != cells.end(); ++theCell) {
        ...
        if (fanout->sinks.size() > MAX_LCB_FANOUTS)
            cout << "  The LCB " << theCell->name << " is driving " << fanout->sinks.size() << " FFs ( >"
                 << MAX_LCB_FANOUTS << " )" << endl;
    }
    return;
}

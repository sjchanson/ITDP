...
bool circuit::measure_timing() {
    ...
    for (vector<unsigned>::iterator PI = PIs.begin(); PI != PIs.end(); ++PI) {
        pin *thePI = &pins[*PI];
        if (thePI->name == clock_port) continue;
    ...
    for (vector<pin>::iterator thePin = pins.begin(); thePin != pins.end(); ++thePin) {
        if (thePin->type == PI_PIN && thePin->name != clock_port) {
            thePin->earlySlk = min(timer.report_slack(thePin->name + "_drv:o", EARLY, RISE),
                                   timer.report_slack(thePin->name + "_drv:o", EARLY, FALL));
            thePin->lateSlk = min(timer.report_slack(thePin->name + "_drv:o", LATE, RISE),
                                  timer.report_slack(thePin->name + "_drv:o", LATE, FALL));

        } else {
            thePin->earlySlk =
                min(timer.report_slack(thePin->name, EARLY, RISE), timer.report_slack(thePin->name, EARLY, FALL));
            thePin->lateSlk =
    ...
    // 				pathlog << "path "<< j <<  ": " << thePin->name << " ( " << thePin->x_coord << ", "<<
    // thePin->y_coord
    // 								<< " ) in net " << nets[ thePin->net ].name << " E:" << thePin->earlySlk << " L:" <<
    // thePin->lateSlk
    // << endl;
    // 			}
...
void circuit::build_steiner() {
    ...
    for (vector<net>::iterator theNet = nets.begin(); theNet != nets.end(); ++theNet) {
        ...
        if (theNet->name == clock_port) continue;
        ...
        if (numpins == 2) {
            ...
            theNet->wire_segs.push_back(
                make_pair(make_pair(pins[theNet->source].name, pins[theNet->sinks[0]].name), wirelength));
        }
        ...
        // otherwise, let's build a FLUTE tree
            ...
            map<pair<unsigned, unsigned>, string>
                pinmap;  // pinmap : map from pair< x location, y location > to pinName
            ...
            unsigned j = 1;
            for (vector<unsigned>::iterator theSink = theNet->sinks.begin(); theSink != theNet->sinks.end();
                 ++theSink, j++) {
            ...
            for (int j = 0; j < branchnum; ++j) {
                ...
                if (pin1 ==
                    pin2)  // NOTE: this only happens when two pin locations are the same (i.e., critically stacked)
                {
                    for (vector<unsigned>::iterator theSink2 = theNet->sinks.begin(); theSink2 != theNet->sinks.end();
                         ++theSink2) {
                        if (pins[*theSink2].name == pin1) continue;
                ...
                if (pin1 != pin2) theNet->wire_segs.push_back(make_pair(make_pair(pin1, pin2), wirelength));
...
void circuit::slice_longwires(unsigned threshold) {
    ...
    cout << "  Slicing wire segments: " << org_tot_wire_segments << " --> " << tot_wire_segments << " ( < "
         << MAX_WIRE_SEGMENT_IN_MICRON << " um )" << endl;
    return;
}
...
void circuit::measure_displacement() {
    ...
    for (vector<cell>::iterator theCell = cells.begin(); theCell != cells.end(); ++theCell)
        if (!theCell->isFixed)
            displacement = max(displacement, fabs(theCell->init_x_coord - theCell->x_coord) +
                                                 fabs(theCell->init_y_coord - theCell->y_coord));
...
void circuit::measure_ABU(double unit, double targUt) {
    ...
    for (vector<cell>::iterator theCell = cells.begin(); theCell != cells.end(); ++theCell) {
        ...
        // non-rectangular shapes
            ...
            for (vector<rect>::iterator theRect = macros[theCell->type].obses.begin();
                 theRect != macros[theCell->type].obses.end(); ++theRect) {
                int lcol = max((int)floor((theCell->x_coord +
                                           (unsigned)(theRect->xLL * static_cast<double>(LEFdist2Microns)) - lx) /
                                          gridUnit),
                               0);
                int rcol = min((int)floor((theCell->x_coord +
                                           (unsigned)(theRect->xUR * static_cast<double>(LEFdist2Microns)) - lx) /
                                          gridUnit),
                               x_gridNum - 1);
                int brow = max((int)floor((theCell->y_coord +
                                           (unsigned)(theRect->yLL * static_cast<double>(LEFdist2Microns)) - by) /
                                          gridUnit),
                               0);
                int trow = min((int)floor((theCell->y_coord +
                ...
                for (int j = brow; j <= trow; j++)
                    for (int k = lcol; k <= rcol; k++) {
                        ...
                        /* get intersection */
                        double lx =
                            max(bins[binId].lx, (double)theCell->x_coord +
                                                    (unsigned)(theRect->xLL * static_cast<double>(LEFdist2Microns)));
                        double hx =
                            min(bins[binId].hx, (double)theCell->x_coord +
                                                    (unsigned)(theRect->xUR * static_cast<double>(LEFdist2Microns)));
                        double ly =
                            max(bins[binId].ly, (double)theCell->y_coord +
                                                    (unsigned)(theRect->yLL * static_cast<double>(LEFdist2Microns)));
                        double hy =
                            min(bins[binId].hy, (double)theCell->y_coord +
                                                    (unsigned)(theRect->yUR * static_cast<double>(LEFdist2Microns)));
        ...
        for (int k = 0; k < x_gridNum; k++) {
            ...
            if (bins[binId].area > gridUnit * gridUnit * BIN_AREA_THRESHOLD) {
                ...
                if (util_array[binId] > 1.0) {
                    ...
                    cout << " m_util: " << bins[binId].m_util << " f_util " << bins[binId].f_util
                         << " free_space: " << bins[binId].free_space << endl;
                    exit(1);
                }
#endif
            }
    ...
    plotfile << "\n";
    plotfile << "set palette defined ( 0 '#000090',\\\n"
                "                      1 '#000fff',\\\n"
                "                      2 '#0090ff',\\\n"
                "                      3 '#0fffee',\\\n"
                "                      4 '#90ff70',\\\n"
    ...
    ABU_penalty = (ABU2_WGT * abu2 + ABU5_WGT * abu5 + ABU10_WGT * abu10 + ABU20_WGT * abu20) /
...
void circuit::write_incremental_spef(const string &output, vector<net> &partial_nets) {
    ...
    for (vector<net>::iterator theNet = partial_nets.begin(); theNet != partial_nets.end(); ++theNet) {
        ...
        for (vector<pair<pair<string, string>, double> >::iterator theSeg = theNet->wire_segs.begin();
            ...
            lumped_cap_at_pin[theSeg->first.first] +=
                theSeg->second / static_cast<double>(DEFdist2Microns) * LOCAL_WIRE_CAP_PER_MICRON * 0.5;
            lumped_cap_at_pin[theSeg->first.second] +=
                theSeg->second / static_cast<double>(DEFdist2Microns) * LOCAL_WIRE_CAP_PER_MICRON * 0.5;
    ...
    for (vector<net>::iterator theNet = partial_nets.begin(); theNet != partial_nets.end(); ++theNet) {
        ...
        feed << ((thePin->type != NONPIO_PIN) ? "*P " : "*I ") << thePin->name
             << ((thePin->type == PI_PIN) ? " I" : " O") << endl;
        for (vector<unsigned>::iterator theSink = theNet->sinks.begin(); theSink != theNet->sinks.end(); ++theSink) {
            pin *thePin = &pins[*theSink];
            feed << ((thePin->type != NONPIO_PIN) ? "*P " : "*I ") << thePin->name
                 << ((thePin->type == PO_PIN) ? " O" : " I") << endl;
        }
        ...
        for (vector<pair<pair<string, string>, double> >::iterator theSeg = theNet->wire_segs.begin();
            ...
            if (!pin_cap_written[theSeg->first.first]) {
                feed << cnt++ << " " << theSeg->first.first << " " << lumped_cap_at_pin[theSeg->first.first] / CAP_SCALE
                     << endl;
                pin_cap_written[theSeg->first.first] = true;
            }
            if (!pin_cap_written[theSeg->first.second]) {
                feed << cnt++ << " " << theSeg->first.second << " "
                     << lumped_cap_at_pin[theSeg->first.second] / CAP_SCALE << endl;
        ...
        for (vector<pair<pair<string, string>, double> >::iterator theSeg = theNet->wire_segs.begin();
             theSeg != theNet->wire_segs.end(); ++theSeg)
            feed << cnt++ << " " << theSeg->first.first << " " << theSeg->first.second << " "
                 << theSeg->second / static_cast<double>(DEFdist2Microns) * LOCAL_WIRE_RES_PER_MICRON / RES_SCALE
                 << endl;
...
void circuit::update_timer_FF2LCB_association(uit::Timer *timer) {
    ...
    if (unique_modified_nets.empty()) return;

...
// extern Tree flautist(int d, DTYPE x[], DTYPE y[], int acc, const uofm::vector<BBox> &obs, const
// uofm::vector<unsigned> &relevantObs, unsigned &legal);
...
#define flutes_wl_LMD(d, xs, ys, s, acc) (d <= D ? flutes_wl_LD(d, xs, ys, s) : flutes_wl_MD(d, xs, ys, s, acc))
